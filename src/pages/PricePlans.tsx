...
import { PricePlan } from "../types/types.ts";

const initialData: PricePlan[] = [
  {
    id: 13334466,
...
const PricePlans: React.FC = () => {
  ...
  const handleUpdate = (updatedPricePlan: PricePlan) => {
    setData(
      data.map((price) =>
        price.id === updatedPricePlan.id ? updatedPricePlan : price
      )
    );
  };
  ...
  return (
    <div>
      <h1 className="title">Price Plan</h1>
      <Table data={data} onEdit={handleUpdate} />
    </div>
  );
};
